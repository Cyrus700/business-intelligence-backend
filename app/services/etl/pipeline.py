...
async def run_frame_pipeline(
    ...
    *,
    ...
    try:
        ...
        load_result = await loader(db, result.records, source_id, job.id)

        if result.records:
            date_field = _DATE_FIELDS[domain]
            dates = [r[date_field] for r in result.records]
            await rebuild_kpi_snapshots(db, min(dates), max(dates))
    ...
    return PipelineResult(
        job_id=str(job.id),
        status=job.status,
        rows_in=job.rows_in or 0,
        rows_loaded=job.rows_loaded or 0,
        rows_rejected=job.rows_rejected or 0,
        skipped_duplicates=job.log["skipped_duplicates"],
        error_report=job.log["error_report"],
    )


async def run_source_pipeline(
    db: AsyncSession, source: DataSource, *, trigger: str
) -> PipelineResult:
    ...
    return await run_frame_pipeline(
        db, source.target_domain, frame, trigger=trigger, source_id=source.id
    )
