...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("retention_days >= 0 AND retention_days <= 365", name=op.f("ck_ai_retention_settings_ck_retention_valid_days")),
        sa.ForeignKeyConstraint(
            ["updated_by"], ["profiles.id"], name=op.f("fk_ai_retention_settings_updated_by_profiles"), ondelete="SET NULL"
        ),
